...
For this repository, use `raw/` as the input corpus and `graphify-out/` as the
generated output directory. Set `GRAPHIFY_OUT=graphify-out`, then build or
refresh it with `/graphify raw --wiki` or `/graphify raw --update --wiki`.
...
if code_files:
    result = extract(code_files, root=Path('INPUT_PATH'), cache_root=Path('.'))
